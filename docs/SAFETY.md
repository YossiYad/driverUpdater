...
The toolbar button becomes available once a scan of this session found something to update. It has the AI research those updates and then installs only the ones it endorsed, without asking the user to judge each one. Updates already reviewed by "Scan with AI" keep their verdict instead of being sent again. It uses the same rules as the AI schedule and the same risk tolerance from Settings > Schedule, so the two never disagree about the same verdict:

- Before anything is installed, a review window lists every update the AI picked with the reason it gave, the pages it checked, and the ones it left out with their reason. Each pick can be unticked, and Cancel installs nothing at all.
...
If the AI research is declined at the usage warning, or the provider is unavailable, nothing is installed at all.
