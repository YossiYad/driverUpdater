...
- Optional scheduled runs (daily/weekly/monthly) with one list of schedule types: off, scan only,
  update every driver, update only the drivers on your own list, or let the AI decide - the run
  reviews every update it found with the configured provider and installs only the ones it
  recommends, up to the risk level you allow.
