...
- **Update with AI.** You choose how to scan, regular or with AI. The button then takes the updates that scan found, has the AI research them for your exact hardware on the web, and installs only the ones it endorses at your configured risk tolerance. A review window shows exactly what it picked and why (and what it left out) before anything is installed, with a "do not show this again" tick for later runs. Anything it cannot rate is left alone.
